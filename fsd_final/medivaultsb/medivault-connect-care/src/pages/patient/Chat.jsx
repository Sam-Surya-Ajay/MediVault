...
const Chat = () => {
  ...
  const fetchDoctors = async () => {
    try {
      // Fetch all doctors for chat sidebar
  ...
  const fetchMessages = async () => {
    ...
    try {
      const data = await chatService.getChatHistory(currentUser.id, selectedDoctor.id);
      setMessages(data);
      if (data.length > 0) {
        await chatService.markMessagesAsRead(selectedDoctor.id, currentUser.id);
      }
    } catch (err) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await chatService.sendMessage(currentUser.id, selectedDoctor.id, message);
      setMessage('');
      fetchMessages();
    } catch (err) {
      toast({
  ...
  return (
    <PatientLayout currentPage="chat">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-200px)]">
        {/* Doctors List */}
        <Card className="border-blue-200">
          <CardHeader>
            <CardTitle className="text-blue-900">Doctors</CardTitle>
          </CardHeader>
          ...
          {selectedDoctor ? (
            <Card className="border-blue-200 h-full flex flex-col">
              <CardHeader className="border-b border-blue-100">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-blue-900">{selectedDoctor.name}</CardTitle>
                    <p className="text-sm text-gray-600">{selectedDoctor.specialty || 'General Physician'}</p>
                  </div>
                </div>
              </CardHeader>
            ...
            <Card className="border-blue-200 h-full flex items-center justify-center">
              <CardContent className="text-center">
                ...
                <p className="text-gray-600">Choose a doctor from the list to begin your conversation</p>
              </CardContent>
            </Card>
          )}
        </div>
